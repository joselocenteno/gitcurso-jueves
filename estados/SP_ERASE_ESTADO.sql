...
DROP PROCEDURE IF EXISTS SP_ERASE_ESTADO $$
CREATE PROCEDURE SP_ERASE_ESTADO(IN userData JSON, OUT rowCount INT)
BEGIN
	...
	BEGIN
		SET vSentencia = CONCAT("ERROR 1217: DELETE FROM estados_tb WHERE id=",data_id);
		INSERT INTO logs_tb (usuarioid, direccionip, plataforma, programa, sentencia, token) VALUES (data_usuarioid, data_direccionip, 1, "SP_ERASE_ESTADO", vSentencia, data_token);
	...
	ELSE
		...
		IF vItems > 1 THEN
			...
			IF FN_CHECKTOKEN(data_token, data_usuarioid) = 1 THEN
				...
				SET vSentencia = CONCAT("DELETE FROM estados_tb WHERE id=",data_id);
				DELETE FROM estados_tb WHERE id = data_id;
